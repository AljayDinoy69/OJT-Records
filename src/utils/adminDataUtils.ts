...
export const createOrUpdateUser = (userData: {
  ...
  email: string, 
  role: UserRole,
  identifier: string // studentId or supervisorId
}): {user: User, password: string} => {
  const users = loadUsers();
  const existingUser = users.find(u => u.id === userData.id);
  
  // Generate default password - using ID to make it predictable but unique
  const defaultPassword = userData.role === 'student' 
    ? `student${userData.identifier.toLowerCase()}` 
    : `supervisor${userData.identifier.toLowerCase()}`;
  ...
  if (existingUser) {
    ...
    saveUsers(users);
    return {user: existingUser, password: existingUser.password};
  } else {
    // Create new user
    const newUser: User = {
      id: userData.id,
    ...
    users.push(newUser);
    saveUsers(users);
    return {user: newUser, password: defaultPassword};
  }
};
